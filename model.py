...
## All enhancement function is inspired from Vxy10 medium blog
...
def preprocess_image_file_train(line_data):
    ...
    if (i_lrc == 0 and line_data['left'][0] != None):
        if(isinstance(line_data['left'][0],float) == False):
            path_file = line_data['left'][0].strip()
            shift_ang = .25
    if (i_lrc == 1):
        path_file = line_data['center'][0].strip()
        shift_ang = 0.
    if (i_lrc == 2 and line_data['right'][0] != None):
        if (isinstance(line_data['right'][0],float) == False):
...
def get_model():
    ...
    #conv1W = K.variable(value=net_data["conv1"][0])
    #conv1b = K.variable(value=net_data["conv1"][1])
    ...
    model.add(Convolution2D(64, filter_size, filter_size,
                            border_mode='valid',
                            name='conv7', init='he_normal'))
    model.add(Activation('relu'))

    model.add(Convolution2D(64, filter_size, filter_size,
                            border_mode='valid',
                            name='conv8', init='he_normal'))
    model.add(Activation('relu'))
    model.add(Convolution2D(64, filter_size, filter_size,
                            border_mode='valid',
                            name='conv9', init='he_normal'))
    model.add(Activation('relu'))
    model.add(Convolution2D(64, filter_size, filter_size,
                            border_mode='valid',
                            name='conv10', init='he_normal'))
    model.add(Activation('relu'))
    model.add(MaxPooling2D(pool_size=pool_size))
    model.add(Dropout(0.5))
    model.add(Convolution2D(128, filter_size, filter_size,
                            border_mode='valid',
                            name='conv11', init='he_normal'))
    model.add(Activation('relu'))
    model.add(Convolution2D(128, filter_size, filter_size,
                            border_mode='valid',
                            name='conv12', init='he_normal'))
    model.add(Activation('relu'))
    model.add(MaxPooling2D(pool_size=pool_size))
    model.add(Dropout(0.5))
    model.add(Flatten())
